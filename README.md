...
- **`index.html`** (ursprünglich `Portfolio.dc.html`, für GitHub Pages umbenannt) — die Seite selbst. Kein reines statisches HTML, sondern ein Template mit `{{ }}`-Platzhaltern und `<sc-for>`/`<sc-if>`-Tags, das erst im Browser gerendert wird.
...
Da `index.html` per `fetch`/Skript nachlädt (`support.js`, `styles.css`), reicht ein direkter Doppelklick auf die Datei (`file://`) je nach Browser nicht — stattdessen über einen einfachen lokalen Webserver öffnen, z. B.:

```bash
python -m http.server 8000
```

Danach im Browser: [http://localhost:8000](http://localhost:8000)
...
- Das Kontaktformular ist als Platzhalter verdrahtet (`handleSubmit` in `index.html`) — für den Live-Betrieb an einen Formular-Dienst wie Formspree oder Resend anbinden.
